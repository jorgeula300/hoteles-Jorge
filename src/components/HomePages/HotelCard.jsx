...
const HotelCard = ({ hotel }) => {
    ...
    const suma = hotel?.reviews.map(item => item.rating).reduce((acc, curr) => acc + curr, 0);


    const raiting = suma / hotel?.reviews.length
    ...
    return (
        <article className=" rounded-xl w-[211px] border shadow">
            <header className=" rounded-t-xl overflow-hidden ">
                <img className="h-[11em] w-full max-w-[211px]" src={hotel.images[0].url} alt={hotel.name} />
            </header>
